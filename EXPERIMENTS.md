...
| Pos84credit | tuning 7: **credit_vertical: true** on the pure-random config; fresh policy, archive seeded from Pos84rand at 12:05 | running (started 2026-09-06 12:05, GPU0) | why: transitive credit required >= 4 bins deeper or another frame, so climbing onto the block (a winning cell in the SAME bin) was never a win for the floor cell; it only won if the episode also reached the pipe top and entered (~6% from the block). Same-bin credit now for higher y-band or another tile signature; flat neighbours still excluded. PREDICTION: 30 min (12:35) first wins from the floor-with-revealed-block cells; 1 h (13:05) wins from the unrevealed floor cell (the reveal link); 2 h (14:05) chain into bins 14-17 and eval > 2600; 4 h (16:05) door-side victory |
